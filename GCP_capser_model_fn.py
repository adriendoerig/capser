...
from GCP_parameters import only_venier, batch_size, only_venier_percent, \
iter_routing, caps1_ncaps, caps1_ndims, caps2_ncaps, caps2_ndims, random_seed, init_sigma, \
batch_norm_conv, alpha_margin, alpha_vernier_reconstruction, alpha_shape_reconstruction, alpha_vernieroffset, \
m_plus, m_minus, lambda_val, n_hidden1, n_hidden2, batch_norm_decoder, n_output, im_size, \
exp_learning_decay, learning_rate, decay_rate, decay_steps, logdir_path
from GCP_parameters import conv1_params, conv2_params, conv3_params
from GCP_capser_functions import \
...
def model_fn(features, labels, mode, params):
    ...
    if only_venier and features['is_training']:
        only_vernier_cond_idx = np.random.choice(batch_size, int(batch_size * only_venier_percent), False)
    ...
    conv_output = conv_layers(X, conv1_params, conv2_params, conv3_params, batch_norm_conv, is_training)
    
    # Create primary caps and their output:
    caps1_output = primary_caps_layer(conv_output, batch_size, caps1_ncaps, caps1_ndims)
    
    # Create secondary caps and their output and also divide vernier caps activation and shape caps activation:
    caps2_output, caps2_output_norm = secondary_caps_layer(caps1_output, batch_size, iter_routing, caps1_ncaps, caps1_ndims, caps2_ncaps, caps2_ndims, random_seed, init_sigma)
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        predictions = {'vernier_offsets': pred_vernierlabels,
                       ...
                       'vernier_accuracy': tf.ones(shape=batch_size) * vernieroffset_accuracy}
        ...
        margin_loss = compute_margin_loss(caps2_output_norm, shapelabels, caps2_ncaps, m_plus, m_minus, lambda_val, batch_size)
        tf.summary.scalar('2_margin_loss', alpha_margin * margin_loss)
        ...
        with tf.name_scope('3_Reconstruction_loss'):
            ...
            vernier_decoder_output, vernier_decoder_output_img = compute_reconstruction(
                    mask_with_labels, shapelabels[:, 0], shapelabels_pred[:, 0], vernier_caps_activation, batch_size, caps2_ndims, n_hidden1, n_hidden2, batch_norm_decoder, n_output, im_size, is_training, '_vernier')
            shape_decoder_output, shape_decoder_output_img = compute_reconstruction(
                    mask_with_labels, shapelabels[:, 1], shapelabels_pred[:, 1], caps2_output, batch_size, caps2_ndims, n_hidden1, n_hidden2, batch_norm_decoder, n_output, im_size, is_training, '_shape')
            ...
            vernier_reconstruction_loss = compute_reconstruction_loss(vernier_images, vernier_decoder_output, batch_size, n_output)
            shape_reconstruction_loss = compute_reconstruction_loss(shape_images, shape_decoder_output, batch_size, n_output)
            reconstruction_loss = vernier_reconstruction_loss + shape_reconstruction_loss

            tf.summary.scalar('vernier_reconstruction_loss', alpha_vernier_reconstruction * vernier_reconstruction_loss)
            tf.summary.scalar('shape_reconstruction_loss', alpha_shape_reconstruction * shape_reconstruction_loss)
            tf.summary.scalar('reconstruction_loss', reconstruction_loss)


        tf.summary.scalar('4_vernieroffset_loss', alpha_vernieroffset * vernieroffset_loss)
        tf.summary.scalar('5_vernieroffset_accuracy', vernieroffset_accuracy)

        # Combine all previously defined losses:
        final_loss = tf.add_n([alpha_margin * margin_loss,
                               alpha_vernier_reconstruction * vernier_reconstruction_loss,
                               alpha_shape_reconstruction * shape_reconstruction_loss,
                               alpha_vernieroffset * vernieroffset_loss],
                              name='final_loss')


        # Training operations: learning rate
        global_step = tf.train.get_global_step()
        if exp_learning_decay:
        ...
        if batch_norm_conv or batch_norm_decoder:
