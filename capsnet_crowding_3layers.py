...
# create datasets
im_size = (60,128)
train_set, train_labels, valid_set, valid_labels, test_set, test_labels = make_crowding_sets(image_size=im_size)
...
plt.imsave(os.path.join(os.getcwd(), 'crowding_sprites.png'),sprites,cmap='gray')

with open(os.path.join(os.getcwd(), 'embedding_labels.tsv'),'w') as f:
    f.write("Index\tLabel\n")
...
caps2_n_caps = 10 # number of capsules
caps2_n_dims = 16 # of 16 dimensions

# it is all taken care of by the function
caps2_output = primary_to_fc_caps_layer(X, caps1_output, caps1_n_caps, caps1_n_dims, caps2_n_caps, caps2_n_dims, print_shapes=False)
...
caps3_n_caps = 2 # number of capsules
caps3_n_dims = 8 # of 8 dimensions

# it is all taken care of by the function
caps3_output = fc_to_fc_caps_layer(X, caps2_output, caps2_n_caps, caps2_n_dims, caps3_n_caps, caps3_n_dims, print_shapes=False)
...
LABELS = os.path.join(os.getcwd(), 'embedding_labels.tsv')
SPRITES = os.path.join(os.getcwd(), 'crowding_sprites.png')
...
checkpoint_path = "./my_capsule_network_3layers"

with tf.Session() as sess:

    summary = tf.summary.merge_all()
    writer = tf.summary.FileWriter('caps_logdir_3layers',sess.graph)
            ...
            for iteration in range(1, n_iterations_per_epoch + 1):
                ...
                if iteration == n_iterations_per_epoch and epoch is n_epochs:
                    ...
                    sess.run(assignment,feed_dict={X: test_set,
                                                   y: batch_labels,
                                                   mask_with_labels: True})
                    saver.save(sess, os.path.join('caps_logdir_3layers','model.ckpt'),epoch)
