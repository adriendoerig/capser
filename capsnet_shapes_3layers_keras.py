...
# create datasets
im_size = (60,128)
use_keras_data_augmentation = True
if use_keras_data_augmentation:

    train_set, train_labels, valid_set, valid_labels, test_set, test_labels = make_shape_sets()
    ...
    data_gen.fit(train_set)
    test_round_size = test_set.shape[0]
    n_rounds = 10
    test_set_augmented = np.ndarray(shape=(test_round_size*n_rounds, test_set.shape[1], test_set.shape[2], test_set.shape[3]),
                         dtype=np.float32)
    test_labels_augmented = np.zeros(test_round_size*n_rounds, dtype=np.float32)
    for test_augmentation_round in range(n_rounds):
        test_set_augmented[test_augmentation_round*test_round_size:(test_augmentation_round+1)*test_round_size,:,:,:], \
        test_labels_augmented[test_augmentation_round:test_augmentation_round+test_round_size] \
            = next(data_gen.flow(test_set,test_labels,batch_size=test_round_size))
    test_set = test_set_augmented
    test_set_augmented = []
else:
    n_repeats = 10
    train_set, train_labels, valid_set, valid_labels, test_set, test_labels = make_shape_sets(image_size=im_size,n_repeats=n_repeats)
...
if show_samples:
    ...
    plt.figure(figsize=(int(np.sqrt(n_samples))+1, int(np.sqrt(n_samples))+1))
    for index in range(n_samples):
        plt.subplot(int(np.sqrt(n_samples))+1, int(np.sqrt(n_samples))+1, index + 1)
        sample_image = test_set[index,:,:].reshape(im_size[0], im_size[1])
...
if use_keras_data_augmentation:
    batch_size = 50
    n_iterations_per_epoch = 200
    n_iterations_validation = 20
else:
    batch_size = 20
...
with tf.Session() as sess:
    ...
    if restore_checkpoint and tf.train.checkpoint_exists(checkpoint_path):
        ...
        for epoch in range(n_epochs):
            if use_keras_data_augmentation:
                iteration = 0
                for batch_data, batch_labels in data_gen.flow(train_set, train_labels, batch_size=batch_size):
                ...
                acc_vals = []
                iteration = 0
                for batch_data, batch_labels in data_gen.flow(valid_set, valid_labels, batch_size=batch_size):
                    # Run the training operation and measure the loss:
                    _, loss_train, summ = sess.run(
                        [training_op, loss, summary],
                        feed_dict={X: batch_data,
