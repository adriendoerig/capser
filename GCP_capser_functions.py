...
################################
def routing_by_agreement(caps2_predicted, batch_size_tensor, iter_routing, caps1_ncaps, caps2_ncaps, caps2_ndims):
    ...
    def routing_body(raw_weights, caps2_output, counter):
        ...
        caps2_output_tiled = tf.tile(caps2_output, [1, caps1_ncaps, 1, 1, 1], name='caps2_output_tiled')
    ...
    with tf.name_scope('Routing_by_agreement'):
        ...
        raw_weights = tf.zeros([batch_size_tensor, caps1_ncaps, caps2_ncaps, 1, 1],
                               dtype=tf.float32, name='raw_weights')
        caps2_output = tf.zeros([batch_size_tensor, 1, caps2_ncaps, caps2_ndims, 1],
...
###################################
def conv_layers(X, conv1_params, conv2_params, conv3_params, batch_norm_conv, phase=True):
...
def primary_caps_layer(conv_output, batch_size, caps1_ncaps, caps1_ndims):
    with tf.name_scope('2_primary_capsules'):
        caps1_reshaped = tf.reshape(conv_output, [batch_size, caps1_ncaps, caps1_ndims], name='caps1_reshaped')
...
def secondary_caps_layer(caps1_output, batch_size, iter_routing, caps1_ncaps, caps1_ndims, caps2_ncaps, caps2_ndims, random_seed, init_sigma):
    with tf.name_scope('3_secondary_caps_layer'):
        ...
        if random_seed:
            W_init = lambda: tf.random_normal(
                shape=(1, caps1_ncaps, caps2_ncaps, caps2_ndims, caps1_ndims),
                stddev=init_sigma, dtype=tf.float32, seed=1, name='W_init')
        else:
            W_init = lambda: tf.random_normal(
                shape=(
                1, caps1_ncaps, caps2_ncaps, caps2_ndims, caps1_ndims),
        ...
        caps1_output_tiled = tf.tile(caps1_output_tile, [1, 1, caps2_ncaps, 1, 1], name='caps1_output_tiled')
    
        # Now we multiply these matrices (matrix multiplication):
        caps2_predicted = tf.matmul(W_tiled, caps1_output_tiled, name='caps2_predicted')
        
        # Routing by agreement:
        caps2_output = routing_by_agreement(caps2_predicted, batch_size, iter_routing, caps1_ncaps, caps2_ncaps, caps2_ndims)
...
def compute_margin_loss(caps2_output_norm, labels, caps2_ncaps, m_plus, m_minus, lambda_val, batch_size):
    with tf.name_scope('compute_margin_loss'):
        ...
        T_shapelabels_raw = tf.one_hot(labels, depth=caps2_ncaps, name='T_shapelabels_raw')
        ...
        present_error = tf.reshape(present_error_raw, shape=(batch_size, caps2_ncaps), name='present_error')
        absent_error_raw = tf.square(tf.maximum(0., caps2_output_norm - m_minus), name='absent_error_raw')
        absent_error = tf.reshape(absent_error_raw, shape=(batch_size, caps2_ncaps), name='absent_error')
        L = tf.add(T_shapelabels * present_error, lambda_val * (1.0 - T_shapelabels) * absent_error, name='L')
...
#       Reconstruction:        #
################################
def compute_reconstruction(mask_with_labels, labels, labels_pred, caps2_output, batch_size, caps2_ndims, n_hidden1, n_hidden2, batch_norm_decoder, n_output, im_size, phase=True, name_extra=None):
    with tf.name_scope('compute_decoder_input'):
        ...
        reconstruction_mask = tf.reshape(reconstruction_mask, [batch_size, 1, current_caps2_ncaps, 1, 1], name='reconstruction_mask')
        caps2_output_masked = tf.multiply(caps2_output, reconstruction_mask, name='caps2_output_masked')
        
        # Flatten decoder inputs:
        decoder_input = tf.reshape(caps2_output_masked, [batch_size, caps2_ndims*current_caps2_ncaps], name='decoder_input')

    # Finally comes the decoder (two dense fully connected ELU layers followed by a dense output sigmoid layer):
    with tf.name_scope('decoder'):
        hidden1 = tf.layers.dense(decoder_input, n_hidden1, use_bias=False, activation=None, name='hidden1' + name_extra)
        ...
        hidden2 = tf.layers.dense(hidden1, n_hidden2, use_bias=False, activation=None, name='hidden2' + name_extra)
        if batch_norm_decoder:
        ...
        decoder_output = tf.layers.dense(hidden2, n_output, activation=tf.nn.sigmoid, name='decoder_output' + name_extra)
        decoder_output_img = tf.reshape(decoder_output, [batch_size, im_size[0], im_size[1], 1],
...
#     Reconstruction loss:     #
################################
def compute_reconstruction_loss(X, decoder_output, batch_size, n_output):
    with tf.name_scope('compute_reconstruction_loss'):
        imgs_flat = tf.reshape(X, [batch_size, n_output], name='imgs_flat')
