...
plt.imsave(os.path.join(os.getcwd(), 'shape_sprites_grouping.png'),sprites,cmap='gray')

with open(os.path.join(os.getcwd(), 'shape_embedding_labels_grouping.tsv'),'w') as f:
    f.write("Index\tLabel\n")
...
caps3_n_caps = 1 # number of capsules
caps3_n_dims = 25 # of n dimensions
...
LABELS = os.path.join(os.getcwd(), 'shape_embedding_labels_grouping.tsv')
SPRITES = os.path.join(os.getcwd(), 'shape_sprites_grouping.png')
...
# create the mask
decoder_input = create_masked_decoder_input(0, 0, caps3_output, caps3_n_caps, caps3_n_dims,
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter('caps_logdir_shapes_grouping',sess.graph)
            ...
            for iteration in range(1, n_iterations_per_epoch + 1):
                ...
                if iteration == n_iterations_per_epoch and epoch is n_epochs:
                    ...
                    sess.run(assignment,feed_dict={X: test_set,
                                                   y: batch_labels,
                                                   mask_with_labels: True})
                    saver.save(sess, os.path.join('caps_logdir_shapes_grouping','model.ckpt'),epoch)
