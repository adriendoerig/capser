...
capser = tf.estimator.Estimator(model_fn=model_fn, model_dir=logdir_path)
train_spec = tf.estimator.TrainSpec(train_input_fn, max_steps=n_steps)
eval_spec = tf.estimator.EvalSpec(eval_input_fn, steps=eval_freq)
...
n_batches = n_test_samples // batch_size * 2


for n_category in range(len(test_data_paths)):
    category = test_data_paths[n_category]
    print('-------------------------------------------------------')
    ...
    txt_file_name = logdir_path + '/uncrowding_exp_results_step_' + str(n_steps) + '.txt'
