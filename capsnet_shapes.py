...
with open(os.path.join(os.getcwd(), 'shape_embedding_labels_2layers.tsv'),'w') as f:
    f.write("Index\tLabel\n")
...
LABELS = os.path.join(os.getcwd(), 'shape_embedding_labels_2layers.tsv')
...
checkpoint_path = "./my_capsule_network_shapes_2layers"

with tf.Session() as sess:

    summary = tf.summary.merge_all()
    writer = tf.summary.FileWriter('caps_logdir_shapes_2layers',sess.graph)
            ...
            for iteration in range(1, n_iterations_per_epoch + 1):
                ...
                if iteration == n_iterations_per_epoch and epoch is n_epochs:
                    ...
                    sess.run(assignment,feed_dict={X: test_set,
                                                   y: batch_labels,
                                                   mask_with_labels: True})
                    saver.save(sess, os.path.join('caps_logdir_shapes_2layers','model.ckpt'),epoch)
