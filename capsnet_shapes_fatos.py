...
plt.imsave(os.path.join(os.getcwd(), 'shape_sprites_fatos.png'),sprites,cmap='gray')

with open(os.path.join(os.getcwd(), 'shape_embedding_labels_fatos.tsv'),'w') as f:
    f.write("Index\tLabel\n")
...
caps3_n_dims = 40 # of n dimensions
...
LABELS = os.path.join(os.getcwd(), 'shape_embedding_labels_fatos.tsv')
SPRITES = os.path.join(os.getcwd(), 'shape_sprites_fatos.png')
...
checkpoint_path = "./my_capsule_network_shapes_fatos"

with tf.Session() as sess:

    summary = tf.summary.merge_all()
    writer = tf.summary.FileWriter('caps_logdir_shapes_fatos',sess.graph)
            ...
            for iteration in range(1, n_iterations_per_epoch + 1):
                ...
                if iteration == n_iterations_per_epoch and epoch is n_epochs:
                    ...
                    sess.run(assignment,feed_dict={X: test_set,
                                                   y: batch_labels,
                                                   mask_with_labels: True})
                    saver.save(sess, os.path.join('caps_logdir_shapes_fatos','model.ckpt'),epoch)
